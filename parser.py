...
def write_to_csv(name:str, data:list):
    with open(name, 'a') as file:
        ...
        for item in data:
            try:
                writer.writerow({'Name' : item['Name'], 'CPU' : item['CPU'], 'CPU SPEED': item['CPU SPEED'], 'RAM': item['RAM'], 
                            'OS': item['OS'], 'VIDEO CARD': item['VIDEO CARD']})
            except: continue


links = get_all_links(get_html(URL))
allData = get_all_data(links)
write_to_csv('test.csv', allData[:100])
